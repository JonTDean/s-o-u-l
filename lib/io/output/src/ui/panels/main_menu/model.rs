...
pub struct ScenarioDraft {
    ...
    pub bg_color: Rgba,
    /// Selected automata from the Classical category (by ID).
    pub selected_classical: Vec<String>,
    /// Selected automata from the Dynamical category (by ID).
    pub selected_dynamical: Vec<String>,
}
...
impl Default for ScenarioDraft {
    fn default() -> Self {
        Self {
            ...
            selected_classical: vec!["wolfram:rule30".to_string()],  // default to Rule 30
            selected_dynamical: Vec::new(),                          // none selected by default
        }
    }
}
