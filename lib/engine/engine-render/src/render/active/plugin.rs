...
use engine_core::{events::{AutomatonAdded, AutomatonId, AutomatonRemoved}, prelude::{AppState, AutomataRegistry}};
use simulation_kernel::{core::cell::CellState, grid::GridBackend};

use crate::{command_executor::focus_camera_on_new_auto, render::camera::systems::{WorldCamera, WORLD_LAYER}, AutomataMaterial, AutomataParams};
...
impl Plugin for ActiveAutomataRenderPlugin {
    fn build(&self, app: &mut App) {
        app.configure_sets(Update, ActiveSet.run_if(in_state(AppState::InGame)))
           ...
           // pipeline: add > remove > upload > sync
           .add_systems(Update, handle_automata_added     .in_set(ActiveSet))
           .add_systems(Update, handle_automata_removed   .in_set(ActiveSet).after(handle_automata_added))
           .add_systems(Update, upload_all_automata       .in_set(ActiveSet).after(handle_automata_removed))
           .add_systems(Update, sync_all_materials        .in_set(ActiveSet).after(upload_all_automata))
           .add_systems(Update, focus_camera_on_new_auto  .in_set(ActiveSet).after(handle_automata_added))
    ...
    for ev in added_events.read() {
        ...
        let mat = materials.add(AutomataMaterial {
            params: AutomataParams {
                ...
                dead_color:   Vec4::new(0.0, 0.0, 0.0, 0.0), // transparent
        ...
        render_map.map.insert(ev.id, (entity, tex.clone(), mat));
