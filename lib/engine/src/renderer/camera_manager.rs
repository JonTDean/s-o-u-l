...
use crate::{
    renderer::{
        components::{WorldCamera, ZoomInfo}, floating_origin::apply_floating_origin, worldgrid::WorldGrid
    }, systems::state::AppState,
};
        ...
        app.init_resource::<ZoomInfo>()
            ...
            .add_systems(
                ...
                (
                    ...
                    end_drag,
                    key_pan,
                    apply_floating_origin,   // keep world around origin
                    refresh_zoom_info,
                )
                    .chain()
...
fn centre_on_world(
    ...
    let (w, h) = match &grid.backend {
        ...
        GridBackend::Sparse(_) => (1_024, 1_024),
    };
