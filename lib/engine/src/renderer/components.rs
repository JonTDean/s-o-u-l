...
use crate::{
    renderer::{floating_origin::{apply_floating_origin, WorldOffset}, worldgrid::WorldGrid}, systems::state::AppState
};
        ...
        app.init_resource::<ZoomInfo>()
            ...
            .add_systems(
                ...
                (
                    ...
                    end_drag,
                    key_pan,
                    apply_floating_origin,            // runs after pan/zoom
                )
                    .chain()
...
fn spawn_once_when_worldgrid_exists(
    ...
    let (w, h) = match &grid.backend {
        ...
        GridBackend::Sparse(_) => (1024, 1024),
    };
