...
/* ───────────── Vertex POD ───────────── */
/// Vertex emitted by the dual‑contouring compute shader.
#[repr(C)]
...
/* ───────────── Constants ───────────── */
/// Work-group size for the dual-contouring dispatch.
pub const WG_SIZE: (u32, u32, u32) = (8, 8, 8);
/// Maximum quads emitted per active voxel.
pub const MAX_QUADS: u32 = 5;
/// Size of one [`Vertex`] in bytes.
pub const BYTES_PER_VERT: u64 = std::mem::size_of::<Vertex>() as u64;
/// Upper bound on voxels processed in one pass.
pub const MAX_VOXELS: u32 = 1024 * 1024;

/* ───────────── Transient buffers ───── */
/// GPU buffers used as scratch space for mesh generation.
#[derive(Resource)]
pub struct MeshletBuffers {
    /// Vertex output buffer.
    pub vertices: Buffer,
    /// Atomic counter tracking emitted vertices.
    pub counter: Buffer,
    /// Indirect draw parameters written by the GPU.
    pub indirect: Buffer,
    /// Total capacity of the vertex buffer in bytes.
    pub capacity: u64,
}
...
/* ───────────── Render-graph node ───── */
/// Render-graph node executing the dual-contouring path.
#[derive(Debug)]
...
impl Node for DualContourNode {
    fn run(
        ...
        let Some(&pid) = cache.map.get("dual_contour") else {
            return Ok(());
        };
        ...
        let bind = device.create_bind_group(
            ...
            &[
                ...
                BindGroupEntry {
                    binding: 2,
                    resource: mesh.counter.as_entire_binding(),
                },
            ],
        );
